<template>
  <q-page class="pagina q-pa-md">
          ...
          <q-form @submit="sendData()" class="q-gutter-md">
            <div class="row">
              <div class="col-md-12 col-xs-12">
                <q-input outlined v-model="form.desc_concepto" label="Descripcion de Concepto" hint="" class="pcform"
                  @update:model-value="
              ...
              <div class="col-md-6 col-xs-6" style="margin-bottom: 7px" id="select">
                <q-field :rules="[(val) => this.$refs.rulesVue.isReq(val, 'Requerido') || '']" hide-bottom-space
                  borderless dense class="pcform" v-model="form.check_comision">
                  <template v-slot:control>
                    <q-checkbox size="lg" v-model="form.check_comision" true-value="1" false-value="0"
                      style="font-size: 13px" label="¿GENERA COMISIÓN?" />
                  </template>
                </q-field>
              </div>
              ...
              <div class="col-md-6 col-xs-6" style="margin-bottom: 7px" id="select">
                <q-field :rules="[(val) => this.$refs.rulesVue.isReq(val, 'Requerido') || '']" hide-bottom-space
                  borderless dense v-model="form.check_impuesto">
                  <template v-slot:control>
                    <q-checkbox size="lg" v-model="form.check_impuesto" true-value="1" false-value="0"
                      style="font-size: 13px" label="¿INFLUYE EN IMPUESTO?" />
                  </template>
                </q-field>
              </div>
            </div>
    ...
    <div class="q-pa-sm justify-center">
      <div class="row q-pa-md">
        ...
        <div class="col-md-5 col-xs-12 col-sm-6 cardMargin selectMobile" style="align-self: center; text-align: center">
          <q-select rounded transition-show="flip-up" transition-hide="flip-down" :options="conceptosSelected" @filter="
            (val, update, abort) =>
              filterArray(
                val,
                update,
          ...
          " use-input hide-selected fill-input input-debounce="0" option-label="desc_concepto" option-value="id"
            v-model="selectedConcepto" outlined standout label="Escoge un Concepto" @update:model-value="
              getData(`/cfacturacion`, 'setData', 'datos')
            ">
            <template v-slot:prepend>
              <q-icon name="search" />
            </template>
        ...
        <div class="col-md-2 col-xl-2 col-lg-2 col-xs-12 col-sm-12" style="text-align: center; align-self: center">
          <q-btn label="Insertar" rounded color="primary" :disabled="this.allowOption(2)"
            @click="dialog = true; this.resetForm()" size="16px" class="q-px-xl q-py-xs insertarestadosmovil"></q-btn>
        </div>
      </div>

      <div class="q-pa-md q-gutter-y-md">
        <q-table :rows="datos" row-key="id" :columns="columns" :loading="loading" :separator="separator"
          :filter="filter" style="width: 100%" :grid="$q.screen.xs" v-model:pagination="pagination">
          ...
          <template v-slot:body-cell-action="props">
            <q-td :props="props">
              ...
              <q-btn dense round flat color="primary" icon="delete" :disabled="this.allowOption(4)"
                @click="selected = props.row.id" @click.capture="deleteForm = true"></q-btn>
            </q-td>
          </template>
                  ...
                  <q-item v-for="col in props.cols" :key="col.name">
                    ...
                    <q-item-section side>
                      ...
                      <q-btn v-else-if="col.name === 'action'" dense round flat color="primary" icon="delete"
                        ...
                        @click.capture="deleteForm = true"></q-btn>
    ...
    <methods ref="methods" @get-Data="getData(`/cfacturacion`, 'setData', 'datos')" @set-Data-Init="setDataInit"
      @reset-Loading="resetLoading" @set-Data-Edit="setDataEdit" @set-Data="setData"
...
export default {
  ...
  mounted() {
    this.$emit("changeTitle", "SCEN - Mantenimiento - Conceptos de Facturacion", "");
  ...
  methods: {
    ...
    sendData() {
      ...
      if (!this.form.id) {
        ...
        this.resetForm();
        this.loading = true; this.dialog = false
      } else {
