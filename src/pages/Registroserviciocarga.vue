<template>
  <q-page class="pagina q-pa-md" style="padding-bottom: 0px; margin-top: -5px">
    ...
    <q-dialog v-model="reversarDialog">
      <q-card style="width: 700px">
        <q-card-section>
          <div class="text-h5" style="font-size: 18px">
...
export default {
  ...
  methods: {
    ...
    reversar() {
      if (this.formEdit.id !== "") {
        if (
          this.formEdit.estatus_administra.value == "F" ||
          this.formEdit.estatus_administra.value == "P" ||
          this.formEdit.estatus_administra == "Pendiente por Facturar" ||
          this.formEdit.estatus_administra == "Pendiente de Cobrar"
        ) {
    ...
    tarificar() {
      if (this.formEdit.id !== "") {
        if (this.detalle_movimiento !== []) {
          if (
            this.formEdit.estatus_administra.value == "A" ||
            this.formEdit.estatus_administra == "Anulada"
          ) {
            this.$q.notify({
              message:
    ...
    setDataEdit(res, dataRes) {
      ...
      if (res.fecha_llega_transito)
        this.formEdit.fecha_llega_transito = res.fecha_llega_transito
          .split("-")
          .reverse()
          .join("/");
    ...
    async putData() {
      this.$refs.formData.validate().then((valid) => {
        if (!valid) {
          ...
          if (formEdit.estatus_operativo == "EN PROCESO DE ENVIÓ")
            formEdit.estatus_operativo = "PR";
          if (formEdit.estatus_administra == "EN ELABORACIÓN")
            formEdit.estatus_administra = "E";
          if (formEdit.pagado_en.value) {
            formEdit.pagado_en = formEdit.pagado_en.value;
          } else {
            formEdit.pagado_en = null;
          }
          ...
          if (formEdit.fecha_elab) {
            formEdit.fecha_elab.split("/").reverse().join("-");
          }

          if (formEdit.id !== "") {
            delete formEdit.porc_comision;
            if (formEdit.id_clte_part_dest == "")
              delete formEdit.id_clte_part_dest;
            if (formEdit.id_clte_part_orig == "")
              delete formEdit.id_clte_part_orig;
            if (this.reversada == true) {
              formEdit.estatus_administra = "E";
        ...
        api
          ...
          .then((res) => {
            ...
            if (this.clienteParticular == "destino") {
              ...
              this.formEdit.id_clte_part_orig = res.data.id;
            }
            return;
          });
      } else {
